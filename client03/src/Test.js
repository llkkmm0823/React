...
/*
거의 모든 문법이 자바스크립트와 비슷한 문법을 사용함.
Camel 케이스 사용
자바스크립트 문법이나 변수를 사용하기 위해서는 {}로 감쌓아서 사용함.
css나 style을 표현할 때는 {{}}로 감쌓아서 사용함
<h1 className="" style={{color:"red", fontSize:"100%"}}
font-size는 {{]}} 안에서 변수의 연산식으로 인식 될 수 있으므로 fonstSize 라고 표현함.
속성명 : "값"으로 표현.  속성들끼리는 ','로 구분
외부의 css 파일을 임포트하여 사용할 수 있음.
가정문 : if else, switch
반복문 : for, map
let Arr[1,2,3];
Arr.map((element. idx)=>{
  return <p>{element}</p>})
return() 안에서 if else 는 사용이 불가능함.
return(
     <div>
              { if( ){}else{}  }  //  X 불가능함.
     </div>
